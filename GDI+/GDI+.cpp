...
void OnPaint(Graphics *m_temp, int x, int y, int n)
{
	char format[5] = ".png";
	char buf[30] = "";
	char num[3];
	...
	m_temp->DrawImage(&picture, 0, 0, x, y);
	if (n < 3) OnPaint(m_temp, x, y, n + 1);
}
...
void OnPaint(HWND hWnd, int x, int y)
{
	HDC hdc = GetDC(hWnd);
	Graphics g(hdc);
	Bitmap* m_buffer = new Bitmap(x, y);
	Graphics *m_temp = new Graphics(m_buffer);
	OnPaint(m_temp, x, y, 1);

	g.DrawImage(m_buffer, 0, 0, x, y);
	delete(m_buffer);
	delete(m_temp);
}
